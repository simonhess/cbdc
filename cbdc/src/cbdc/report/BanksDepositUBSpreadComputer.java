 ...
 *
 */
public class BanksDepositUBSpreadComputer extends AbstractMicroComputer implements MicroMultipleVariablesComputer {
...
private int banksId;
	...
	public Map<Long, Double> computeVariables(MacroSimulation sim) {
		...
		for (Agent i:pop.getAgents()){
			Bank bank= (Bank) i;
			if (!bank.isDead()){
				double depositUpperBoundSpread = (bank.getPassedValue(StaticValues.LAG_DEPOSITUPPERBOUND, 1)*bank.getPassedValue(StaticValues.LAG_DEPOSITS, 1)) - bank.getTotInterestsDeposits();
				result.put(bank.getAgentId(), (depositUpperBoundSpread));
			}
			else{
